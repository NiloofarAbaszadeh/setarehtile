    ...
    return (<>
      ...
      />
        ...
        <div style={props.style} className='bg-[#f7f7f7]'>
            <Helmet>
                <title>{language === "fa-IR" ? "کلکسیون ها | کاشی و سرامیک ستاره  " : language === "en" ? "Collection | Setareh Tiles & Ceramic" : "کلکسیون ها | کاشی و سرامیک ستاره  "}</title>
            </Helmet>
        {boardData && 
            <div className="h-[400px] lg:h-[580px] md:h-[550px] sm:h-[500px] xs:h-[380px] overflow-hidden relative">
                ...
                <Parallax className="lg-no-parallax bg-cover absolute -top-[50px] left-0 w-full h-[100vh] z-0 lg:-top-[70px] md:h-[600px] md:-top-[50px] xs:h-[450px]" translateY={[-40, 40]} style={{ backgroundImage: `url(${host}${boardData.collectionImage.data.attributes.formats.custom.url})` }}></Parallax>
                <Container className="h-full relative z-[2]">
                  <Row className="justify-center h-full">
                    <Col xl={6} lg={7} md={8} className="relative  text-center flex justify-center flex-col">
                      <h2 className="text-white font-sans -tracking-[1px] mb-0 text-[50px]">{boardData.collectionTitle}</h2>
                    </Col>
                  </Row>
                </Container>
            </div>}
