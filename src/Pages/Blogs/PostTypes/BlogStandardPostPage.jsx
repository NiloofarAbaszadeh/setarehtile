  ...
  useEffect(() => {
    ...
    const newsElements = () => {
      if (url[1] === "news") {
        ...
        axios.get(`${host}/api/all-comments?populate=deep`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        .then(res => {
                  ...
                  <Row>
                    <Col className="blog-details-text last:mb-0 mb-8">
                      ...
                      <ul className="flex mb-8 xs:block mb-[2rem]">
                        ...
                        {data[0] && (
                            <li className="inline-block text-gray align-middle"><i className="feather-user text-fastblue mx-[10px]"></i>نوشته شده توسط {data[0].attributes.author.data.attributes.fullName}</li>
                          )}
                      </ul>
                      {/* the first image */}
                      ...
                      {data[0].attributes.likes && (
                          <div className="text-center md:text-end px-0 flex justify-end sm:justify-center">
                            <Link aria-label="link" className="uppercase text-darkgray text-xs w-auto font-medium inline-block border border-mediumgray rounded pt-[5px] pb-[6px] px-[18px] leading-[20px] hover:text-black transition-default hover:shadow-[0_0_10px_rgba(23,23,23,0.10)] " to="#">
                              ...
                              </i><span>{data[0].attributes.likes} لایک</span>
                            </Link>
                          </div>)}
                    </Col>
                    <Col>
            ...
            <div className='box-shadow rounded-[10px] bg-white w-max pt-8 mt-8'>
              <Row className="justify-center">
                  <Col lg={9} className="mb-8 sm:mb-8">
                      ...
                      <div className="mb-[5px]">ایمیل شما منتشر نخواهد شد. قسمت های مورد نیاز علامت زده شده اند <span className="text-red">*</span></div>
                  </Col>
              </Row>
              <Row className="justify-center">
                  <Col lg={9}>
