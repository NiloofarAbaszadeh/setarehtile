...
const ImageGalleryPage = () => {
  ...
  const url = "internal-ce/"
  
  useEffect(() => {
    const GetData = async () => {
      await axios.get(`${host}/api/gwahy-dakhlies?populate=deep&locale=${language}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      .then(res => {
        setData(res.data.data)
      })
      await axios.get(`${host}/api/internal-certificates?populate=deep&locale=${language}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      .then(res => {
  ...
  return (<>
      <Helmet>
        <title>{language === "fa-IR" ? "گواهی های داخلی | کاشی و سرامیک ستاره میبد" : language === "en" ? "Internal CE | Setareh Meybod Tile & Ceramic" : ""}</title>
      </Helmet>
      ...
      <Parallax className="lg-no-parallax bg-cover absolute -top-[50px] left-0 w-full h-[100vh] z-0 lg:-top-[70px] md:h-[600px] md:-top-[50px] xs:h-[450px]" translateY={[-40, 40]} style={{ backgroundImage: `url(${host}${data[0].attributes.boardImage.data.attributes.formats.custom.url})` }}></Parallax>
