        ...
        <ul className={`${props.theme}${props.className ? ` ${props.className}` : ""}`}>
            {
                props.data.map((item, i) => {
                    return (
                        <m.li className='' key={i}  {...{ ...props.animation, transition: { delay: (i+1) * props.animationDelay, initial: { opacity: 0, x: "100%" }, ease: [0.25, 1, 0.5, 1], duration: 1.2 } }}>
                            ...
                            {item.title ?
                                <div className='block'>
                                    ...
                                    <span className='text-darkgray text-[13px]'>{item.content}</span>
                                )}
