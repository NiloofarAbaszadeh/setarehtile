...
const PortfolioBoxed03 = (props) => {
    ...
    return (<>
    {!props.data && <div className='flex justify-center items-center bg-white w-full h-[100vh] fixed top-0 fix z-50 top-[-25px]'>
        <ScaleLoader
        color={"#db1010"}
        loading={!props.data}
        size={50}
                                ...
                                <Link to={`./${item.attributes.name}`} aria-label="link">
                                    ...
                                    >
                                        <div className="portfolio-image relative bg-white h-full w-full ">
                                            {item.attributes.image.data.attributes.formats.medium.url && <img className="rounded-[2px]" src={host + item.attributes.image.data.attributes.formats.medium.url} height={447} width={550} alt="portfolio-boxed" />}
