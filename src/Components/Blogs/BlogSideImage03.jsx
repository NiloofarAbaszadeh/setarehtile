...
const BlogSideImage03 = (props) => {
  ...
  useEffect(() => {
    setLoading(false)
    const GetData = () => {
      axios.get(`${host}/api/catalog-infos?populate=deep`, {
  ...
  return (<>
    <Helmet>
      <title> کاتالوگ ها | کاشی و سرامیک ستاره  </title>
    </Helmet>
                  ...
                  return (
                    <m.div key={i} className={`blog-sideimage p-0 m-5 `} {...props.animation}>
                      ...
                      <div className="post-details my-8 px-12">
                        ...
                        <Link to={host + item.file.data.attributes.url}><button className="button-custom w-max mt-6 sm:mb-2 sm:mx-8">دانلود فایل </button></Link>
                      </div>
                    </m.div>
                  );
                })
