        ...
        authorDetails && (
            <m.div {...fadeIn} className={`flex justify-center items-center w-full shadow-[0_0_15px_rgba(0,0,0,0.1)] rounded-[5px] p-16 sm:block${className ? ` ${className}` : ""}`}>
                <div className="w-[130px] mr-[60px] sm:mx-auto text-center">
                    ...
                    <Link aria-label="link" to={`/blogs/author/${authorId}`} className="text-darkgray font-medium mt-[20px] block text-md hover:text-fastblue">{authorDetails.name}</Link>
                ...
                <div className="w-[75%] md:text-start sm:w-full sm:text-center">
                    ...
                    <Buttons ariaLabel="authorbox" to={`/blogs/author/${authorId}`} className="font-medium after:h-[2px] after:bg-black hover:text-black uppercase btn-link md:mb-[15px]" size="md" color="#232323" title="All author posts" />
                </div>
            </m.div>
        )
    )
