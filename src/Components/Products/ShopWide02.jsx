    ...
    useEffect(() => {
        const GetData = async () => {
            if (props.data[0]) {
                ...
                await axios.get(`${host}/api/products?filters\[baseInfo\][size]=${props.data[0]}&filters\[baseInfo\][color_themes]=${props.data[1]}&populate=*&pagination[pageSize]=4&locale=${language}`, {
                ...
                // eslint-disable-next-line
                await axios.get(`${host}/api/products?filters\[baseInfo\][color_themes]=${props.data[1]}&populate=*&pagination[pageSize]=4&locale=${language}`, {
                    headers: { Authorization: `Bearer ${token}` }
                  }).then(res => {
                    setData(res.data.data)
                  })
          ...
          GetData()
    }, [host, token, props.data, language])
        ...
        {data && <div className='flex items-center justify-center flex-row'>
            {
               data.map((item, i) => {
                   return (
                       <div key={i}
                           className="product-box w-[280px] relative overflow-hidden mb-[45px] xs:mb-0 m-4">
                           <Link aria-label="link" to={`/product-tilse/${item.attributes.name}`} target="_blank">
                               <div className="product-image p-5 relative overflow-hidden rounded-md bg-[#EAE9E9]">
